...
const Wrapper = styled.div`
    ...
    ${mobile({ padding: '10px', margin: '5vh 0vw !important'})}
    ${tablet2({ padding: '10px', margin: '20vh 0vw'})}
`
...
const Checkout = () => {
    ...
    useEffect(() => {
        const getProvinces = async () => {
            try {
                ...
                setProvinces(res.data.data.sort((a,b) => a.name.localeCompare(b.name)));
            } catch (err) {
                console.log(err)
            }
        }
    ...
    {/*useEffect(() => {
        if (selectedProvince) {
            const getCities = async () => {
                try {
                    const res = await axios.get(`https://apis.datos.gob.ar/georef/api/localidades?provincia=${selectedProvince}&max=100`)
    ...
    useEffect(() => {
        setOrder({
            ...order, 
            paymentMethod: currency === 'pesos' ? 'mercadopago' : 'paypal', 
            amount: currency === 'pesos' ? cart.total : cart.dolarTotal, 
    ...
    const handleTabChange = (tab) => {
        ...
        if (tab === 0) { 
            ...
            setDiscount(order?.amount * 0.1) 
        } else { 
            setOrder({...order, paymentMethod: 'paypal', amount: cart.dolarTotal})
            setDiscount(0)
        }
    }
    ...
    const handleEmailChange = (e) => {
        if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(e.target.value)) {
            ...
            setErrorText({
                ...errorText, email: 'La dirección electrónica no es una dirección válida',
            });
        }
    }
    ...
    const handlePersonalIdChange = (e) => {
        if (e.target.value.length <= 15 && e.target.value.length >= 5) {
            setError({
            ...
            setErrorText({
                ...errorText, personalId: 'El número identificacion no debe ser entre 5 y 15 caracteres',
            });
        }
    }
    ...
    const handlePhoneChange = (e) => {
        if (e.target.value.length < 20 && e.target.value.length > 7) {
            ...
            setErrorText({
                ...errorText, phone: 'El número de teléfono ser menor a 7 caracteres ni mayor a 20',
            });
    ...
    const handleMercadoPago = async () => {
        ...
        await publicRequest.post(`${BASE_URL}/payments/course-checkout`, order)
    ...
    const handleTransference = async () => {
        ...
        window.location.href = 'https://cursos.lanuderia.com/pending';
    ...
    const handlePaypal = async () => {
        ...
        try{
            const saveOrder = () => dispatch(createOrder(order))
            const orderSaved = await saveOrder().unwrap().then((resData) => dispatch(orderEmail(resData)))
        ...
        await publicRequest.post(`${BASE_URL}/payments/paypal/create-order`, cart)
        ...
        <Wrapper>
            ...
            <Top>
                <TopButton to='/cart'>Volver al carrito</TopButton>
                    <Stepper stepNum={2} />
                <TopButton type="filled" to='/products'>Continuar Comprando</TopButton>
            </Top>
            <Bottom>
                <Left>
                    <DataContainer>
                        ...
                        <Form>
                            ...
                            <TextField 
                                ...
                                style={{flex: '1', '&::-webkit-outer-spin-button': {
                ...
                <Right>
                    <Summary>
                        ...
                        <SummaryItem>
                            ...
                            <SummaryItemText>$ {new Intl.NumberFormat('de-DE').format(order?.amount)}</SummaryItemText>
                        </SummaryItem>
                        <SummaryItem>
                        ...
                        : (order?.paymentMethod === 'mercadopago' ? <FinishButton type="filled" onClick={handleMercadoPago}>PAGAR CON <Logo src='https://firebasestorage.googleapis.com/v0/b/la-nuderia.appspot.com/o/mercadopagoButton.png?alt=media&token=85836135-4828-48fd-a90c-b0ec56c78169' alt='MercadoPago' /></FinishButton>
                        : order?.paymentMethod === 'transference' ? <FinishButton type="filled" onClick={handleTransference}>PAGAR CON TRANSFERENCIA</FinishButton>
                        : order?.paymentMethod === 'paypal' ? <FinishButton type="filled" onClick={handlePaypal}>PAGAR CON <Logo src='https://www.paypalobjects.com/digitalassets/c/website/marketing/apac/C2/logos-buttons/optimize/Online_Primary_Acceptance_Mark_RGB_V2_small.jpg' alt='PayPal' /></FinishButton> : '') 
                        }
